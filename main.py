...
async def _notify_new_lead(lead: dict, staff: dict):
    ...
    # branch_line — показываем филиал в сообщении, только если у компании больше одного
    # активного филиала (для однофилиальной компании это лишняя информация — и так ясно,
    # куда едет заявка). Готовая строка с переводом строки — просто исчезает из шаблона,
    # когда не нужна (тот же приём, что и у location_link ниже).
    branch_line = ""
    if branch:
        try:
            branches = await db.get_branches(lead.get("company_id") or 1)
            if len([b for b in branches if b["active"]]) > 1:
                branch_line = f"🏢 {branch_ru(branch)}\n"
        except Exception as e:
            logging.warning(f"_notify_new_lead branch_line error: {e}")

    # Кр. адрес приоритетнее полного — по решению пользователя (то же поле, что видно
    # в карточке лида/заказа как "Кр. адрес").
    ...
    try:
        ...
        if result.get("ok") and lead_id and keyboard:
...
async def _tg_mark_lead_taken(lead_id: int, staff_name: str, gender: str = ""):
    ...
    прямо в Telegram. Редактирует то же сообщение: убирает кнопку и дописывает
    "✅ Взял(а): Имя", как и при нажатии кнопки (зеркало прод-фикса)."""
    if not db.pool:
        return
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT company_id, tg_chat_id, tg_message_id, tg_msg_text FROM leads WHERE id=$1", lead_id)
    if not row or not row["tg_chat_id"] or not row["tg_message_id"] or not row["tg_msg_text"]:
        return
    token = await db.get_config_for_company("order_bot_token", row["company_id"]) if row["company_id"] else None
    if not token:
        return
...
# ── Логистика / Регионы обслуживания (перенос из ARTEZ, company-scoped —
# company_id читается внутри db-функций через _cid(), явно нигде не передаётся) ──
...
# через region_id. company_id (не project, как в ARTEZ) изолирует между компаниями.
@app.get("/api/admin/places")
async def list_places_ep(region_id: int, staff=Depends(get_current_staff)):
    if staff.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Только для admin")
    places = await db.list_places_by_region(region_id)
...
async def create_place_ep(
    ...
    if staff.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Только для admin")
    place = await db.create_place(region_id, category_ru, category_uz, name_ru, name_uz, lat, location_address, note)
...
# ── Справочник категорий точек — редактируемый самой компанией (см. places выше).
@app.get("/api/admin/place-categories")
async def list_place_categories_ep(staff=Depends(get_current_staff)):
    if staff.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Только для admin")
    cats = await db.list_place_categories()
...
async def create_place_category_ep(
    ...
    if staff.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Только для admin")
    cat = await db.create_place_category(name_ru, name_uz, icon, sort_order)
